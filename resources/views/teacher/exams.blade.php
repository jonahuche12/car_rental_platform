...
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-md-10">
            <div class="mb-4">
                <button class="btn btn-primary btn-lg btn-block" data-toggle="modal" data-target="#createexamModal">Create New Exam</button>
            </div>
                        ...
                        @foreach ($academicSession->terms->sortByDesc('created_at') as $term)
                            ...
                            <div class="term-card border rounded mb-3 ml-3">
                                <div class="term-header {{ $termColorClass }} p-2" data-toggle="collapse" data-target="#term-{{ $term->id }}" aria-expanded="true" aria-controls="term-{{ $term->id }}">
                                    ...
                                    <span class="badge bg-primary">{{ $sortedexams->count() }}</span>
                                </div>
    ...
    <td colspan="6">
        <div class="collapse" id="exam-{{ $exam->id }}">
            <table class="table table-striped nested-table bg-light">
                ...
                <tbody>
                    @if($exam->academicSession->id == $school->academicSession->id)
                    ...
                    @else
                        @foreach ($exam->grades as $grade)
                            <tr>
                                ...
                                <td>
                                    @php
                                        $percentage = \App\Models\Grade::calculatePercentage($grade->score, $exam->complete_score ?? 0);
                                    @endphp
                                    {{ $percentage }}%
                                </td>
                                <td>
...
@endforeach
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-primary text-light">
                        <h5 class="modal-title" id="createexamModalLabel">Create New <b>{{$course->name}} </b> exam for <b><span id="class-name">{{$class_section->name}}</span></b></h5>
        ...
        <div class="modal fade" id="archiveConfirmationModal" tabindex="-1" role="dialog" aria-labelledby="archiveConfirmationModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header  bg-primary text-light">
                        <h5 class="modal-title bg-primary text-light" id="archiveConfirmationModalLabel">Archive exam Confirmation</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
